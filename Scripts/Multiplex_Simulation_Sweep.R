##############################################
#
#   Multiplex Simulation Analyses - Full sweep 
#
#############################################
# Make data
N_datasets = 70
...
Samp = round(exp(seq(2.35, 4.98,length.out=N_test)),0)
Layers = seq(2, 8, length.out=N_layers_top)

# Just some random labels for the layers
jungian_archetypes =  c("Ruler",
                        "Creator",
                        "Sage",
                        "Innocent",
                        "Explorer",
...
dr_list = vector("list", N_layers_top)
make_Dr = function(K){
  if(K>1){
  dr_Rho_A = runif(K, -1, 1)
...
G_list[[(layers-1)*N_test + samp]] = sr_Rho
D_list[[(layers-1)*N_test + samp]] = dr_Rho
N_list[[(layers-1)*N_test + samp]] = Samp[samp]
L_list[[(layers-1)*N_test + samp]] = Layers[layers]
...
outcome = vector("list", N_layers)
exposure = vector("list", N_layers)
...
names(outcome) = jungian_archetypes[1:N_layers]
names(exposure) = jungian_archetypes[1:N_layers]
...
dat$node_count = N_id
dat$layer_count = N_layers

# Store the STRAND data for cholsky mode
dat_list_cholesky[[(layers-1)*N_test + samp]] = dat
dat_list_cholesky[[(layers-1)*N_test + samp]]$sr_Rho = sr_Rho
dat_list_cholesky[[(layers-1)*N_test + samp]]$dr_Rho = dr_Rho
dat_list_cholesky[[(layers-1)*N_test + samp]]$model_to_deploy = 1
M_list[[(layers-1)*N_test + samp]]$model = "cholesky"

# Store the STRAND data for l2norm mode
dat_list_l2norm[[(layers-1)*N_test + samp]] = dat
dat_list_l2norm[[(layers-1)*N_test + samp]]$sr_Rho = sr_Rho
dat_list_l2norm[[(layers-1)*N_test + samp]]$dr_Rho = dr_Rho
dat_list_l2norm[[(layers-1)*N_test + samp]]$model_to_deploy = 2
M_list[[(layers-1)*N_test + samp]]$model = "l2norm"
...
ggsave("RunTimeAbs_DR.pdf", p1, height=5, width=6)
...
ggsave("ESS_Bulk_DR.pdf", p1, height=5, width=6)
...
ggsave("ESS_Tail_DR.pdf", p1, height=5, width=6)
...
ggsave("Frob_DR.pdf", p1, height=5, width=6)
...
ggsave("Efficiency.pdf", p1, height=5, width=6)

#################################### Loop over data sets and make calculations
G_layers = G_SampSize = G_res_f_test_m = G_res_f_test_l = G_res_f_test_h = matrix(NA, nrow=N_layers_top, ncol=11)
D_layers = D_SampSize = D_res_f_test_m = D_res_f_test_l = D_res_f_test_h = matrix(NA, nrow=N_layers_top, ncol=11)

for(i in 1:N_datasets){
  n = which(Samp == N_list[[i]])
  l = which(Layers == L_list[[i]])
  ...
  G_layers[l,n] = Layers[l]
  D_layers[l,n] = Layers[l]
...
for(i in 1:N_layers_top){
 scrap_g = test_F_norm_prior(G_list[[i*N_test]])
 scrap_d = test_F_norm_prior(D_list[[i*N_test]])

 G_res_f_test_m[i,11] = scrap_g[1]
 D_res_f_test_m[i,11] = scrap_d[1]

 G_res_f_test_l[i,11] = scrap_g[2]
 D_res_f_test_l[i,11] = scrap_d[2]

 G_res_f_test_h[i,11] = scrap_g[3]
 D_res_f_test_h[i,11] = scrap_d[3]

 G_layers[i,11] = Layers[i]
 D_layers[i,11] = Layers[i]

 G_SampSize[i,11] = "Random"
 D_SampSize[i,11] = "Random"
}
...
ggsave("Frobenius_Norms_Bernoulli_cholesky.pdf",p, width=9, height=5.5)
...
ggsave("Frobenius_Norms_Bernoulli_l2norm.pdf",px, width=9, height=5.5)
